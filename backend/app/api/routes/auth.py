...
def login(payload: LoginRequest) -> LoginChallenge:
    user_record = DEMO_USERS.get(
        (payload.institution_id, payload.username)
    )

    if user_record is None or payload.password != user_record["password"]:
        raise HTTPException(
            status_code=401,
            detail="Invalid institution ID, username, or password",
        )

    if payload.terminal_id not in TERMINALS:
        raise HTTPException(
            status_code=403,
            detail="Terminal is not enrolled",
        )

    mfa_token = auth_store.create_login_challenge(
        f"{payload.institution_id}:{payload.username}"
    )
...
def verify_otp(payload: OtpVerifyRequest) -> AuthSession:
    if not auth_store.verify_login_challenge(
        payload.mfa_token,
        payload.otp,
    ):
        raise HTTPException(
