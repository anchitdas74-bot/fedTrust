...
export const LoginPortal: React.FC = () => {
  ...
  // Step 1 Form state
  const [institutionId, setInstitutionId] = useState('BANK-A');
  const [username, setUsername] = useState('analyst');
  const [password, setPassword] = useState('fedtrust123');
  ...
  const handleOTPSubmit = async (
    ...
    try {
      ...
      // Some AuthContext implementations return false
      // instead of throwing on verification failure.
      if (!success) {
        ...
        if (remaining === 0) {
          ...
          setLocalOtpError(
            `Incorrect OTP. ${remaining} attempt${remaining === 1 ? '' : 's'
            } remaining.`
          );
        }
      }
    } catch (err: any) {
      // Backend returns 401 for an invalid/expired OTP.
      const remaining = Math.max(
        0,
        attemptsLeft - 1
      );
      ...
      if (remaining === 0) {
        ...
        setLocalOtpError(
          `${backendMessage} ${remaining} attempt${remaining === 1 ? '' : 's'
          } remaining.`
        );
      }
    } finally {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-extrabold text-xl tracking-wider text-gray-900 dark:text-white font-mono">
                FedTrust
              </span>
            ...
            <p className="text-xs text-gray-400">
              Secure FinTech Fraud Intelligence Portal
            </p>
          </div>
        </div>
                ...
                <div className="p-3 rounded-xl bg-gray-950/60 border border-gray-800 flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400">
                        Threshold: 0.0450 Loss
                      </p>
                    </div>
                  </div>

                  <span
                    className={`px-2 py-0.5 text-[10px] font-mono font-bold rounded ${mlServiceStatus === 'active'
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 text-xs font-mono font-bold mb-3">
                    ...
                    STEP 1 / 2: CREDENTIAL VERIFICATION
                  </div>
              ...
              <div className="space-y-6 animate-in fade-in duration-300">
                ...
                <div>
                  ...
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/30 text-xs font-mono font-bold mb-3">
                    ...
                    STEP 2 / 2: 6-DIGIT OTP VERIFICATION
                  </div>

                  <h1 className="text-2xl font-bold text-white">
                    Security Officer 2FA
                  </h1>

                  <p className="text-xs text-gray-400 mt-1">
                    Enter the 6-digit OTP sent to your registered security device.
                  </p>

                </div>

                {/* Error */}
                {(localOtpError || otpError) && (
                ...
                >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full text-center font-mono text-3xl tracking-[0.5em] py-3 bg-gray-950 border border-gray-700 rounded-xl text-cyan-300 focus:outline-none focus:border-cyan-500 transition-all placeholder:tracking-normal placeholder:text-sm placeholder:text-gray-600 disabled:opacity-50"
                      autoFocus
                    />
                  ...
                  <p className="text-[11px] text-gray-400 italic bg-gray-950/60 p-2.5 rounded-lg border border-gray-800">
                    Enter the 6-digit OTP sent to your registered security device. The code expires after 3 minutes and is limited to 3 attempts.
                  </p>
